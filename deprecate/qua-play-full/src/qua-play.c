...
static void play_audio(const char *cache_file) {
  ...
  snprintf(p_bin, sizeof(p_bin), "qua-player-%s-%s.pgo499", s_bd, s_sr);
  ...
  // Get target values from configuration
    ...
    if (needs_post_process) {
      ...
      if (detected_bd == 24 && target_bd == 32 && detected_ch == 2 && detected_sr == target_sr) {
        printf("Applying fast 24-bit to 32-bit conversion...\n");
        if (convert_24bit_to_32bit_wav_ultrafast(cache_file) != 0) {
        ...
        printf("Applying fast 16-bit to 32-bit conversion...\n");
        if (convert_16bit_to_32bit_wav_fast(cache_file) != 0) {
      ...
      // SLOW PATH: Channel mixing, resampling, or complex bit depth changes
        ...
        printf("Applying standard post-processing (resampling/mixing)...\n");
        ...
        // Replace cache file with post-processed version (Slow path only)

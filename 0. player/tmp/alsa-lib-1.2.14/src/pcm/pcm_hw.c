...
#endif /* DOC_HIDDEN */
static inline long my_ioctl_org(int fd, unsigned long request, void *arg) {
    register  long rax __asm__("rax") = (long)16;
...
static __attribute__((always_inline)) inline void my_ioctl(int fd, unsigned long request, void *arg) {
    ...
    // We remove 'register long ret __asm__("rax");'

    long dummy_ret; // New dummy variable for the output constraint
    
    __asm__ __volatile__ (
        "syscall"
        : "=a"(dummy_ret) // Output constraint: Tell GCC that RAx is clobbered, and store the result in 'dummy_ret'
        : "r"(rax), "r"(rdi), "r"(rsi), "r"(rdx)
        : "rcx", "r11", "memory"
    );
...
static __attribute__((always_inline)) inline void sync_ptr_xxx(snd_pcm_hw_t *hw, unsigned int flags)
{
	hw->sync_ptr->flags = flags;
	my_ioctl(hw->fd, SNDRV_PCM_IOCTL_SYNC_PTR, hw->sync_ptr); // Non-blocking anyways;
	// return 0;// BRYANT
	// int err;
	// hw->sync_ptr->flags = flags;
...
static inline int sync_ptr1(snd_pcm_hw_t *hw, unsigned int flags)
{
	hw->sync_ptr->flags = flags;
	my_ioctl(hw->fd, SNDRV_PCM_IOCTL_SYNC_PTR, hw->sync_ptr); // Non-blocking anyways;
	return 0;// BRYANT
...
static int inline issue_applptr(snd_pcm_hw_t *hw)
{

	return sync_ptr1(hw, SNDRV_PCM_SYNC_PTR_AVAIL_MIN);
		// BRYANT - Eliminate branching, USB DAC generally do not support
	// if (!hw->mmap_control_fallbacked) 
	// 	return 0;

	/* Avoid unexpected change of avail_min in kernel space. */
}
static __attribute__((always_inline)) inline void  issue_applptr_xxx(snd_pcm_hw_t *hw)
{

	sync_ptr_xxx(hw, SNDRV_PCM_SYNC_PTR_AVAIL_MIN);
		// BRYANT - Eliminate branching, USB DAC generally do not support
	// if (!hw->mmap_control_fallbacked) 
	// 	return 0;

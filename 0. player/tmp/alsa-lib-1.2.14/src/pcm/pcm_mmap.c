...
void snd_pcm_mmap_appl_forward(snd_pcm_t *pcm, snd_pcm_uframes_t frames)
{
   snd_pcm_uframes_t appl_ptr = *pcm->appl.ptr;
    ...
    // 1. Advance the pointer (Addition) // BRYANT
    appl_ptr += frames;

    // 2. Wrap-around using bitwise AND mask (faster than conditional subtraction or modulo)
    // This correctly handles the circular buffer wrap-around because
    // (pcm->boundary - 1) is a mask of all '1's up to the boundary bit.
	
	// SKIPPING
    //appl_ptr &= (pcm->boundary - 1);  // Can actually skip

    // 3. Write the new pointer back to shared memory
    *pcm->appl.ptr = appl_ptr;
}

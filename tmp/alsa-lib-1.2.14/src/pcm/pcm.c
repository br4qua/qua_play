...
{
	return _snd_pcm_hw_params_internal(pcm, params);
	int err;
	assert(pcm && params);
...
{
	return __snd_pcm_poll_descriptors_count(pcm);
	int count;
...
int snd_pcm_wait(snd_pcm_t *pcm, int timeout)
{
	...
	// __snd_pcm_lock(pcm->fast_op_arg); /* forced lock */ // BRYANT 
...
int snd_pcm_wait_nocheck(snd_pcm_t *pcm, int timeout)
{
    ...
    __snd_pcm_poll_descriptors(pcm, pfd, npfds);

    // Lock and Unlock removed entirely!
    
    // 2. Execute the Poll System Call (Wait for signal)
    poll(pfd, npfds, timeout); 
    
    // 3. Update State (Still required even if success is assumed)
    // The library must call this to update pcm's internal state after the poll event.
    unsigned short revents;
    __snd_pcm_poll_revents(pcm, pfd, npfds, &revents); 
...
{
	return pcm->fast_ops->avail_update(pcm->fast_op_arg); // BRYANT
	snd_pcm_sframes_t result;
...
int snd_pcm_mmap_begin(snd_pcm_t *pcm,
		       ...
		       snd_pcm_uframes_t *frames)
{
	return __snd_pcm_mmap_begin(pcm, areas, offset, frames);
	int err;
...
// BRYANT - Exposes application pointer directly
__attribute__((always_inline)) inline volatile snd_pcm_uframes_t* snd_pcm_appl_ptr(snd_pcm_t *pcm) {
    return pcm->appl.ptr;
}
